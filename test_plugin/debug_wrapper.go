...
func main() {
	// Read the todos/get.go file
	source, err := os.ReadFile("../resources/todos/get.go")
	if err != nil {
	...
	// Print the wrapper with line numbers
	...
	if len(lines) >= 61 {
		fmt.Printf("Line 61: %s\n", lines[60])
	} else {
		fmt.Printf("Warning: Generated code has only %d lines\n", len(lines))
	}
}
